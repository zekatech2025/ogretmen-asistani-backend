...
- Madde numaralarını ve belge adlarını mutlaka belirt
...
async def get_user_quota(user_id: str) -> tuple[int, int]:
    """Kullanıcının kullandığı ve kalan prompt sayısını döner."""
    result = sb.table("user_quotas").select("*").eq("user_id", user_id).execute()

    if not result.data:
        # İlk kez kullanan kullanıcı — kayıt oluştur
...
async def chat(request: Request, body: ChatRequest):
    ...
    # RAG — ilgili belge parçalarını getir
